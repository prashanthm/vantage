...
// React + LookeyDS are page globals; all data is mock (src/data.js).
import {
  TODAY, ACCOUNTS, MARKET, LOTS, RECENT_BUYS, AUTO_BUYS, PARTNER_MAP,
  WASH_FAMILIES, OVERLAP_GROUPS, WASH_WINDOW_DAYS, TICKER_STRIP, AI_INSIGHTS,
  AI_PICKS, SIGNALS, SECTORS, NOTIFICATIONS_SEED, NOTIF_TYPES, CHAT_RULES,
  ALLOCATION_TARGETS, ASSET_CLASSES,
} from "./data.js";
...
const signUsd = (n) => `${n >= 0 ? "+" : "−"}${usd(Math.abs(n))}`;
const signPct = (n, d = 2) => `${n >= 0 ? "+" : "−"}${Math.abs(n).toFixed(d)}%`;
const cls = (...xs) => xs.filter(Boolean).join(" ");
const dirCls = (n) => (n > 0 ? "up" : n < 0 ? "down" : "");

const DAY_MS = 86400000;
const daysAgo = (iso) => Math.floor((TODAY - new Date(iso + "T12:00:00")) / DAY_MS);
...
const lotValue = (l) => l.shares * MARKET[l.symbol].price;
const lotCost = (l) => l.shares * l.costPerShare;
const lotUnrl = (l) => lotValue(l) - lotCost(l);
...
const accountValue = (id) => LOTS.filter((l) => l.account === id).reduce((s, l) => s + lotValue(l), 0);
    ...
    <div>
      ...
      <div className="vg-shell">
        <div className="vg-main">
            ...
            <div className="vg-card">
              ...
              <button className={cls("vg-acct", accountId === "all" && "sel")} onClick={() => setAccountId("all")}>
                <div>
                  ...
                  <div className="meta">{ACCOUNTS.length} linked · consolidated</div>
                </div>
              ...
              {ACCOUNTS.map((a) => (
                <button key={a.id} className={cls("vg-acct", accountId === a.id && "sel")} onClick={() => setAccountId(a.id)}>
                  <div>
                    <div>{a.name}</div>
                    <div className="meta">{a.type} · synced {a.lastSync}</div>
                  </div>
                  <span className="bal">{usd(accountValue(a.id))}</span>
                </button>
              ))}
              <p className="vg-note" style={{ marginTop: 12 }}>
                Linked read-only via aggregator (demo). Vantage never holds funds or places orders.
              </p>
            </div>

            <div className="vg-card">
              <div className="vg-kicker">Why cross-account?</div>
              <p style={{ fontSize: 13.5, color: "var(--color-grey)", margin: "0 0 10px" }}>
                Wash sales, overlap, and allocation drift only show up when every account is viewed together —
                a buy in one account can void a tax loss harvested in another.
              </p>
              <button className="vg-linkbtn" onClick={() => { setNotifOpen(true); }}>
                {unread ? `${unread} unread alert${unread > 1 ? "s" : ""} →` : "View notifications →"}
              </button>
            </div>
          </div>
          ...
          <div>
            {/* Portfolio overview */}
            <section className="vg-section" id="portfolio">
                    ...
                    return (
                      <span key={k}>
                        ...
                        {accountId === "all" && Math.abs(drift) >= 3 && (
                          <span className={cls("vg-badge", drift > 0 ? "warn" : "info")}>
                            {signPct(drift, 1)} vs target
                          </span>
                        )}
                      </span>
                    );
            ...
            {/* Holdings */}
            <section className="vg-section" id="holdings">
              <h2>Holdings</h2>
              <p className="vg-sub">
                    ...
                    {pos.map((p) => (
                      <React.Fragment key={p.symbol}>
                        <tr className="click" onClick={() => setExpanded((e) => ({ ...e, [p.symbol]: !e[p.symbol] }))}>
                          ...
                          <td>
                            {[...p.accounts].map((id) => (
                              <span className="vg-chip" key={id}>{ACCOUNTS.find((a) => a.id === id).short}</span>
                            ))}
                          </td>
                        ...
                        {expanded[p.symbol] && p.lots.map((l, i) => (
                          <tr className="vg-subrow" key={i}>
                            ...
                            <td>{ACCOUNTS.find((a) => a.id === l.account).short}</td>
            ...
            {/* Tax Center */}
            <section className="vg-section" id="tax">
              <h2>Tax Center — loss harvesting</h2>
              <p className="vg-sub">
              ...
              <div className="vg-card vg-tablewrap" style={{ padding: "8px 12px" }}>
                <table className="vg-table">
                  <thead>
                    <tr>
                      <th>Lot</th><th>Account</th><th className="num">Unrealized</th>
            ...
            {/* Recommendations */}
            <section className="vg-section" id="recs">
            ...
            {/* Markets */}
            <section className="vg-section" id="markets">
                ...
                <div className="vg-heat">
                  {SECTORS.map((sec) => (
                    <div className="vg-heat-sector" key={sec.name}>
                      <h4>
                        {sec.name}
                        <span className={dirCls(sec.pct)} style={{ color: sec.pct >= 0 ? "var(--vg-success-deep)" : "var(--vg-danger)" }}>
                          {signPct(sec.pct)}
                        </span>
                      </h4>
      ...
      {notifOpen && (
        <NotifPanel
          notifs={notifs} setNotifs={setNotifs} settings={settings} saveSettings={saveSettings}
  ...
  return (
    <Modal title={`${stock.sym} — analysis`} open onClose={onClose}>
      <div className="vg-row" style={{ marginBottom: 12 }}>
        ...
        {held.length > 0
          ? <span className="vg-badge info">You hold this in {[...new Set(held.map((l) => ACCOUNTS.find((a) => a.id === l.account).short))].join(", ")}</span>
