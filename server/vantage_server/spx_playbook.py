...
def write_pine_file(scaffold: dict) -> str | None:
    ...
    ``pine/`` dir (env ``VANTAGE_PINE_DIR``; default ``<repo>/pine``). Returns the
    path written, or None if there was nothing to render. Called by the nightly
    job and the /recompute route so the copy-paste artifact stays current."""
    import os
