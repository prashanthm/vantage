...
def gex_snapshot() -> dict:
    """Latest dealer-gamma snapshot: flip / walls / max_pain / regime / ladder.

    Shape (Sentinel gex_intel.py): ``{spot, net_gex_bn, regime, regime_text,
    gamma_flip, call_wall, put_wall, max_pain, call_share_pct, put_share_pct,
    ladder[], curve[], narrative[], note, date, generated_at, n_contracts}``.
    """
...
def gex_history() -> list[dict]:
    """Nightly GEX rows (for the regime→next-day-vol edge). May be empty."""
...
def pull_all(day: str, next_day: str | None = None) -> dict:
    """Everything the playbook needs from Sentinel, each independently graceful.
