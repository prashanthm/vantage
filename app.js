...
${ref}`;
  ...
  function InstrumentChartCard({ symbol, defaultTf = "15m", overlays, height }) {
    ...
    return /* @__PURE__ */ React.createElement(InstrumentChart, { symbol, tf, setTf, overlays, height });
  }
  ...
  function ExitsView({ refreshNonce }) {
    const [data, setData] = useState8({ positions: [], broker: [], live_gate: false });
    const [busy, setBusy] = useState8(false);
    const [actions, setActions] = useState8(null);
  ...
  function TodayView({ refreshNonce }) {
    ...
    const [pb, setPb] = useState10(null);
    const [pos, setPos] = useState10(null);
    const [ticket, setTicket] = useState10(null);
    const load = async () => {
  ...
  function WhyCard({ pb, onReload }) {
    ...
    const [pine, setPine] = useState10(null);
    const [pineTitle, setPineTitle] = useState10("TradingView Pine");
  ...
  // src/journal.jsx
  var { useState: useState13, useRef: useRef4, useEffect: useEffect10, useMemo: useMemo5 } = React;
